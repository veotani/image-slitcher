...
'''Linear function used in linear gradient mask creation'''
...
'''Getting borders of common area'''
...
'''Find closest point to list of points (points is (x,y) tuple)'''
def closest_node(point, points):
    deltas = points - point
    dist_2 = np.einsum('ij,ij->i', deltas, deltas)
    return np.argmin(dist_2)

'''Decompose image by areas (only img1, only img2 and img1&&img2) and represent result as mask'''
...
def main(image1, image2):
    ...
    mask[zone_a_log] = 0
    disp_borders_a = np.zeros(borders.shape)
    disp_borders_b = disp_borders_a.copy()
    disp_borders_a[zone_a_log&borders] = 255
    disp_borders_b[zone_b_log & borders] = 255
    cv2.imwrite("intermediate/borders.png", disp_borders)
    cv2.imwrite("intermediate/borders_a.png", disp_borders_a)
    cv2.imwrite("intermediate/borders_b.png", disp_borders_b)
    ...
    for point_c in tqdm(zone_c):
        ...
        # mask[point_c[0], point_c[1]] = da / (da + db) * 255.0
        if da >= db:
    ...
    # Если изображение было слишком большое, возвращаем маску назад
    if img_height > 1000 and img_width > 1000:
        mask = scipy.misc.imresize(mask, (img_height, img_width))
